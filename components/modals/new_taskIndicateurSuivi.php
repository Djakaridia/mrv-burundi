<!-- modal -->
<div class="modal fade" id="suiviIndicTaskModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    aria-hidden="true">
            ...
            <div class="modal-body px-2">
                ...
                <div id="suiviTaskIndicFormContainer" class="d-none">
                    <form id="suiviTaskIndicForm" style="min-height: 300px; max-height: 400px; overflow-y: auto;">
                        <input type="hidden" id="suiviIndicId">
                        ...
                        <table class="table table-sm table-hover table-striped fs-12 table-bordered border-emphasis" align="center">
                            ...
                            <tbody id="rowSuiviIndicTaskForm">
                            </tbody>
                        </table>
                ...
                <div id="suiviIndicTacheContainer">
                    ...
                    <div class="card border rounded-0 overflow-hidden">
                        <div class="card-body p-1 table-responsive scrollbar" style="height: 300px; overflow-y: auto;">
                            <table class="table table-sm table-hover table-striped fs-9 table-bordered border-emphasis"
                                ...
                                <tbody id="rowSuiviIndicTacheTab" class="list">
                    ...
                    <!-- Message vide -->
                    <div id="emptySuiviIndicTask" class="text-center py-6 d-none">
...
<script>
    ...
    $(document).ready(function() {
        $('#suiviIndicTaskModal').on('shown.bs.modal', async function(event) {
    ...
    function showSuiviTaskAddForm() {
        ...
        $('#suiviIndicTacheContainer').addClass('d-none');
    ...
    function cancelSuiviTaskForm() {
        ...
        $('#suiviIndicTacheContainer').removeClass('d-none');
        loadSuivisIndicateur();
    }

    async function loadSuivisIndicateur() {
        $('#rowSuiviIndicTacheTab').empty();
        try {
            ...
            if (result.data && result.data.length > 0) {
                result.data.forEach(item => {
                    ...
                    $('#rowSuiviIndicTacheTab').append(row);
                });
            } else {
                $('#rowSuiviIndicTacheTab').append('<tr><td colspan="2" class="text-center p-10">Aucun suivi trouvé.</td></tr>');
            }
        } catch (error) {
        ...
        try {
            ...
            if (result.data) {
                result.data.forEach(item => {
                    ...
                    $('#rowSuiviIndicTaskForm').append(row);
                });

                $('#suiviTaskIndicFormContainer').removeClass('d-none');
                $('#suiviIndicTacheContainer').addClass('d-none');
            } else {
                $('#rowSuiviIndicTaskForm').append('<tr><td colspan="2" class="text-center p-10">Aucun indicateur trouvé.</td></tr>');
            }
        } catch (error) {
